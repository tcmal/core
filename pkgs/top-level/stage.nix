...
let
  ...
  byNamePackages = self: super:
    let
      ...
      res = {
        ...
        // lib.foldl (acc: path:
          (import "${path}/packages.nix" { inherit lib noSysDirs config; } res
            self super) // acc) { } packageGroupFiles;
    in res;

  basePackages = self: super:
    let
      res =
        import ./base-packages.nix { inherit lib noSysDirs config; } res self
        super;
    in res;
  ...
  buildSupportPackages = self: super:
    let
      res =
        import ../build-support/packages.nix { inherit lib noSysDirs config; }
        res self super;
    in res;
