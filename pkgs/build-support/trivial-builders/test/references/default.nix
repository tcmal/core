{ lib, stdenvNoCC, testers, callPackage, writeText
# nativeBuildInputs
, shellcheck-minimal
# Samples
...
# Test targets
, writeDirectReferencesToFile, writeClosure }:
...
let
  ...
  samplesToString = attrs:
    ...
    (name: "[${name}]=${lib.escapeShellArg "${attrs.${name}}"}")
  ...
  testScriptBin = stdenvNoCC.mkDerivation (finalAttrs: {
    ...
    installPhase = ''
      ...
      substitute "$src" "$out/bin/${finalAttrs.meta.mainProgram}" \
        ...
        --replace "@CLOSURES@" ${
          lib.escapeShellArg (samplesToString closures)
        } \
        --replace "@DIRECT_REFS@" ${
          lib.escapeShellArg (samplesToString directReferences)
        } \
    ...
    passthru = { inherit collectiveClosure directReferences closures samples; };

    meta = with lib; { mainProgram = "references-test"; };
  });
in testers.nixosTest {
  ...
  nodes.machine = { ... }: {
    ...
    environment.etc."pre-built-paths".source =
      writeText "pre-built-paths" (builtins.toJSON [ testScriptBin ]);
  };
  testScript = ''
    machine.succeed("""
  ...
  meta = { maintainers = with lib.maintainers; [ roberth ShamrockLee ]; };
}
