{ lib, fetchurl, stdenv, callPackages, runCommand }:
...
let
  ...
  fetchModule = { module, npmRoot ? null }:
    (if module ? "resolved" then
      (let
        # Parse scheme from URL
        mUrl = match "(.+)://(.+)" module.resolved;
        scheme = elemAt mUrl 0;
      in (if mUrl == null then
        (assert npmRoot != null; { outPath = npmRoot + "/${module.resolved}"; })
...
in {
  importNpmLock = { npmRoot ? null
    , package ? importJSON (npmRoot + "/package.json")
    , packageLock ? importJSON (npmRoot + "/package-lock.json")
    , pname ? getName package, version ? getVersion package }:
    let
      mapLockDependencies = mapAttrs (name: version:
        (
          ...
          if (
            ...
            matchGitHubReference version != null) then
      ...
      packageLock' = packageLock // {
        packages = mapAttrs (_: module:
          let src = fetchModule { inherit module npmRoot; };
          in (removeAttrs module [ "link" "funding" ])
          ...
          // lib.optionalAttrs (module ? dependencies) {
            ...
            optionalDependencies =
              mapLockDependencies module.optionalDependencies;
    ...
    in runCommand "${pname}-${version}-sources" {
