{ lib, stdenvNoCC, rustPlatform, makeWrapper, pkg-config, curl, gnutar, gzip
, testers, fetchurl, cacert, prefetch-npm-deps, fetchNpmDeps }:

{
  prefetch-npm-deps = rustPlatform.buildRustPackage {
    ...
    passthru.tests = let
      ...
      makeTest =
        { name, src, hash, forceGitDeps ? false, forceEmptyCache ? false }:
    ...
    in {
      ...
      emptyCache = makeTest {
        ...
        src = fetchurl {
          url =
            "https://raw.githubusercontent.com/bufbuild/protobuf-es/v1.2.1/package-lock.json";
      ...
      jitsiMeet9111 = makeTest {
        ...
        src = fetchurl {
          url =
            "https://raw.githubusercontent.com/jitsi/jitsi-meet/stable/jitsi-meet_9111/package-lock.json";
  ...
  fetchNpmDeps = { name ? "npm-deps", hash ? "", forceGitDeps ? false
    , forceEmptyCache ? false, ... }@args:
    let
      hash_ = if hash != "" then {
        outputHash = hash;
      } else {
