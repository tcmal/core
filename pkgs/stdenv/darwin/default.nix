...
let
  ...
  stageFun = prevStage:
    ...
    let
      cc = if prevStage.llvmPackages.clang-unwrapped == null then
        null
      else
        lib.makeOverridable (import ../../build-support/cc-wrapper) {
          ...
          expand-response-params = lib.optionalString
            (prevStage.stdenv.hasCC or false && prevStage.stdenv.cc
              != "/dev/null") prevStage.expand-response-params;

          extraPackages = [ prevStage.llvmPackages.compiler-rt ];

          extraBuildCommands = let
            inherit (prevStage.llvmPackages)
              clang-unwrapped compiler-rt release_version;
          in ''
...
[
  ...
  (prevStage:
    stageFun prevStage {
      ...
      overrides = self: super: {
        ...
        darwin = super.darwin.overrideScope (selfDarwin: superDarwin:
          {
            ...
            binutils-unwrapped = bootstrapTools // { version = "boot"; };
        ...
        llvmPackages = super.llvmPackages // (let
  ...
  (prevStage:
    ...
    assert lib.all isFromBootstrapFiles (with prevStage;
      [ bash coreutils cpio gnugrep ]
    ...
    assert (!useAppleSDKLibs)
      -> lib.all isFromBootstrapFiles (with prevStage.darwin; [ Libsystem ]);
    assert useAppleSDKLibs
      -> lib.all isFromNixpkgs (with prevStage.darwin; [ Libsystem ]);
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        ...
        cmakeMinimal = super.cmakeMinimal.overrideAttrs
          (old: { buildInputs = old.buildInputs ++ [ self.darwin.CF ]; });
        ...
        python3Minimal = super.python3Minimal.overrideAttrs
          (old: { buildInputs = old.buildInputs ++ [ self.darwin.CF ]; });
  ...
  (prevStage:
    ...
    assert (!useAppleSDKLibs) -> lib.all isBuiltByBootstrapFilesCompiler
      (with prevStage.darwin; [ Libsystem configd ]);
    assert (!useAppleSDKLibs)
      -> lib.all isFromNixpkgs (with prevStage.darwin; [ CF ]);
    assert useAppleSDKLibs -> lib.all isFromNixpkgs
      (with prevStage.darwin; [ CF Libsystem libobjc ]);
    assert lib.all isFromNixpkgs (with prevStage.darwin; [ dyld launchd xnu ]);
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        inherit (prevStage)
          ccWrapperStdenv autoconf automake bash binutils binutils-unwrapped
          bison brotli cmake cmakeMinimal coreutils cpio cyrus_sasl db ed expat
          flex gettext gmp gnugrep groff icu libedit libffi libiconv libidn2
          libkrb5 libssh2 libtool libunistring libxml2 m4 ncurses nghttp2 ninja
          openldap openssh openssl patchutils pbzx perl pkg-config
          python3Minimal scons sed serf sharutils sqlite subversion texinfo
          unzip which xz zlib zstd;
        ...
        darwin = super.darwin.overrideScope (_: superDarwin: {
          inherit (prevStage.darwin)
            CF sdkRoot Libsystem binutils-unwrapped cctools cctools-port configd
            darwin-stubs dyld launchd libclosure libdispatch libobjc locale
            objc4 postLinkSignHook print-reexports rewrite-tbd signingUtils
            sigtool;
  ...
  (prevStage:
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        inherit (prevStage)
          ccWrapperStdenv autoconf automake bash binutils binutils-unwrapped
          bison brotli cmake cmakeMinimal cpio cyrus_sasl db ed expat flex
          gettext gmp groff icu libedit libffi libiconv libidn2 libkrb5 libssh2
          libtool libunistring libxml2 m4 ncurses nghttp2 ninja openldap openssh
          openssl patchutils pbzx perl pkg-config python3 python3Minimal scons
          sed serf sharutils sqlite subversion sysctl texinfo unzip which xz
          zlib zstd;

        # Switch from cctools-port to cctools-llvm now that LLVM has been built.
        darwin = super.darwin.overrideScope (_: superDarwin: {
          inherit (prevStage.darwin)
            CF Libsystem configd darwin-stubs dyld launchd libclosure
            libdispatch libobjc locale objc4 postLinkSignHook print-reexports
            rewrite-tbd signingUtils sigtool;
          ...
          cctools-llvm =
            superDarwin.cctools-llvm.override { enableManpages = false; };
        });
        ...
        llvmPackages = super.llvmPackages // (let
          ...
          libraries = super.llvmPackages.libraries.extend (selfLib: superLib: {
            compiler-rt = null;
            libcxx =
              superLib.libcxx.override ({ stdenv = libcxxBootstrapStdenv; });
          });
  ...
  (prevStage:
    ...
    assert lib.all isFromNixpkgs
      (with prevStage.darwin; [ dyld launchd libclosure libdispatch xnu ]);

    assert lib.all isBuiltByBootstrapFilesCompiler
      (with prevStage.llvmPackages; [ clang-unwrapped libclang libllvm llvm ]);
    assert lib.all isBuiltByNixpkgsCompiler
      (with prevStage.llvmPackages; [ libcxx ]);
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        inherit (prevStage)
          ccWrapperStdenv autoconf automake binutils-unwrapped bison brotli
          cmake cmakeMinimal coreutils cpio cyrus_sasl db ed expat flex gettext
          gmp gnugrep groff icu libedit libffi libiconv libidn2 libkrb5 libssh2
          libtool libunistring libxml2 m4 ncurses nghttp2 ninja openbsm openldap
          openpam openssh openssl patchutils pbzx perl pkg-config python3
        ...
        darwin = super.darwin.overrideScope (selfDarwin: superDarwin: {
          inherit (prevStage.darwin)
            CF binutils-unwrapped cctools configd darwin-stubs launchd libobjc
            libtapi locale objc4 print-reexports rewrite-tbd signingUtils
            sigtool;
        ...
        llvmPackages = super.llvmPackages // (let
          tools = super.llvmPackages.tools.extend (_: _: {
            inherit (prevStage.llvmPackages)
              clang-unwrapped clangNoCompilerRtWithLibc libclang libllvm llvm;
          });
        ...
        stdenv =
          let stdenvNoCF = super.stdenv.override { extraBuildInputs = [ ]; };
          in self.overrideCC stdenvNoCF
          (self.llvmPackages.clangNoCompilerRtWithLibc.override {
  ...
  (prevStage:
    ...
    assert lib.all isBuiltByBootstrapFilesCompiler
      (with prevStage; [ libffi libiconv libxml2 ncurses zlib zstd ]);
    ...
    assert (!useAppleSDKLibs) -> lib.all isBuiltByBootstrapFilesCompiler
      (with prevStage.darwin; [ configd ]);
    assert (!useAppleSDKLibs) -> lib.all isBuiltByNixpkgsCompiler
      (with prevStage.darwin; [ Libsystem ]);
    assert (!useAppleSDKLibs)
      -> lib.all isFromNixpkgs (with prevStage.darwin; [ CF ]);
    assert useAppleSDKLibs -> lib.all isFromNixpkgs
      (with prevStage.darwin; [ CF Libsystem libobjc ]);
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        inherit (prevStage)
          ccWrapperStdenv autoconf automake bash bison brotli cmake cmakeMinimal
          coreutils cpio cyrus_sasl db ed expat flex gettext gmp gnugrep groff
          libedit libidn2 libkrb5 libssh2 libtool libunistring m4 ncurses
          nghttp2 ninja openbsm openldap openpam openssh openssl patchutils pbzx
          perl pkg-config python3 python3Minimal scons serf sqlite subversion
          sysctl texinfo unzip which xz zstd;
        ...
        darwin = super.darwin.overrideScope (selfDarwin: superDarwin: {
          inherit (prevStage.darwin)
            Libsystem configd darwin-stubs launchd locale print-reexports
            rewrite-tbd signingUtils sigtool;
        ...
        llvmPackages = super.llvmPackages // (let
          ...
          libraries = super.llvmPackages.libraries.extend (selfLib: superLib: {
            ...
            compiler-rt = superLib.compiler-rt.override ({
              ...
              stdenv =
                self.stdenv.override { extraBuildInputs = [ self.darwin.CF ]; };
            });
          });
        in {
          inherit tools libraries;
        ...
        stdenv =
          ...
          (self.llvmPackages.clangNoCompilerRtWithLibc.override {
            ...
            bintools =
              self.llvmPackages.clangNoCompilerRtWithLibc.bintools.override {
                libc = self.darwin.Libsystem;
              };
          });
      };
  ...
  (prevStage:
    ...
    assert lib.all isBuiltByBootstrapFilesCompiler
      (with prevStage.darwin; [ locale print-reexports rewrite-tbd sigtool ]);
    assert lib.all isBuiltByNixpkgsCompiler
      (with prevStage.darwin; [ binutils-unwrapped cctools libtapi ]);
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        inherit (prevStage)
          ccWrapperStdenv autoconf automake bash binutils binutils-unwrapped
          bison brotli cmake cmakeMinimal coreutils cpio cyrus_sasl db ed expat
          flex gettext gmp gnugrep groff libedit libidn2 libkrb5 libssh2 libtool
          libunistring m4 nghttp2 ninja openbsm openldap openpam openssh openssl
          patchutils pbzx perl pkg-config python3 python3Minimal scons sed serf
          sharutils sqlite subversion sysctl texinfo unzip which xz zstd
        ...
        darwin = super.darwin.overrideScope (selfDarwin: superDarwin: {
          inherit (prevStage.darwin)
            CF Libsystem binutils binutils-unwrapped cctools cctools-llvm
            cctools-port configd darwin-stubs dyld launchd libclosure
            libdispatch libobjc libtapi locale objc4 postLinkSignHook
  ...
  (prevStage:
    ...
    stageFun prevStage {
      ...
      overrides = self: super: {
        inherit (prevStage)
          ccWrapperStdenv autoconf automake bash bison cmake cmakeMinimal
          cyrus_sasl db expat flex groff libedit libtool m4 ninja openldap
          openssh patchutils perl pkg-config python3 scons serf sqlite
          subversion sysctl texinfo unzip which
        ...
        darwin = super.darwin.overrideScope (selfDarwin: superDarwin: {
          inherit (prevStage.darwin)
            dyld CF Libsystem darwin-stubs
            # CF dependencies - don’t rebuild them.
            libobjc objc4;
          ...
          signingUtils =
            superDarwin.signingUtils.override { inherit (selfDarwin) sigtool; };
                    ...
                    let
                      clangVersion =
                        if lib.versionAtLeast clangRelease "16" then
                          ...
                          clangRelease;
                    in "${clangLib}/lib/clang/${clangVersion}/include";
                in ''
                  rsrc="$out/resource-root"
                  mkdir "$rsrc"
                  ln -s "${
                    clangResourceRootIncludePath clang-unwrapped.lib
                    release_version
                  }" "$rsrc"
                  ln -s "${compiler-rt.out}/lib"   "$rsrc/lib"
                  ln -s "${compiler-rt.out}/share" "$rsrc/share"
    ...
    in {
      ...
      stdenv = import ../generic lib {
        ...
        overrides = self: super:
          {
            inherit (prevStage)
              bash binutils brotli bzip2 coreutils cpio diffutils ed file
              findutils gawk gettext gmp gnugrep gnumake gnused gnutar gzip icu
              libffi libiconv libidn2 libssh2 libunistring libxml2 libyaml
              ncurses nghttp2 openbsm openpam openssl patch pbzx pcre
              python3Minimal xar xz zlib zstd;

            darwin = super.darwin.overrideScope (_: superDarwin:
              {
                inherit (prevStage.darwin)
                  CF ICU Libsystem darwin-stubs dyld locale libobjc libtapi
                  rewrite-tbd xnu;
              ...
              (super.stdenv.targetPlatform == localSystem) {
                inherit (prevStage.darwin)
                  binutils binutils-unwrapped cctools-llvm cctools-port;
              });
            ...
            llvmPackages = super.llvmPackages // (let
              tools = super.llvmPackages.tools.extend (_: _: {
                inherit (prevStage.llvmPackages)
                  clang clang-unwrapped libclang libllvm llvm;
              });
              libraries = super.llvmPackages.libraries.extend (_: _: {
                inherit (prevStage.llvmPackages) compiler-rt libcxx;
              });
