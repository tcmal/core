...
let
  inherit (builtins)
    isString isInt isAttrs isList all any attrValues isFunction isBool
    concatStringsSep isFloat;
  isTypeDef = t:
    isAttrs t && t ? name && isString t.name && t ? verify
    && isFunction t.verify;
    ...
    in {
      ...
      verify =
        ...
        if t == self.any then
          ...
          attrs: isAttrs attrs && all verify (attrValues attrs);
    };
