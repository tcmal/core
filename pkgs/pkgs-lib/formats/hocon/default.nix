...
in {
  ...
  format = { generator ? hocon-generator, validator ? hocon-validator
    ...
    let
      hoconLib = {
        mkInclude = value:
          ...
          in assert lib.assertMsg (lib.elem includeStatement.type [
            "file"
            "url"
            "classpath"
            null
        ...
        mkSubstitution = value:
          if lib.isString value then {
            ...
            assert lib.assertMsg (lib.isAttrs value) ''
              Value of invalid type provided to `hocon.lib.mkSubstition`: ${
                lib.typeOf value
              }
            '';
    ...
    in {
      type = let
        type' = with lib.types;
          let atomType = nullOr (oneOf [ bool float int path str ]);
          in (oneOf [ atomType (listOf atomType) (attrsOf type') ]) // {
      ...
      generate = name: value:
        ...
        in callPackage
        ({ stdenvNoCC, hocon-generator, hocon-validator, writeText }:
