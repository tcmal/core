{ lib, stdenv, fetchFromGitHub, Security, autoreconfHook, util-linux, openssl
, cacert
...
, variant ? "all", extraConfigureFlags ? [ ], enableLto ?
...
stdenv.mkDerivation (finalAttrs: {
  ...
  configureFlags = [ "--enable-${variant}" "--enable-reproducible-build" ]
    ++ lib.optionals (variant == "all") [
      ...
      "--enable-sp=yes${
        lib.optionalString
        (stdenv.hostPlatform.isx86_64 || stdenv.hostPlatform.isAarch) ",asm"
      }"
      "--enable-sp-math-all"
      "--enable-harden"
  ...
  nativeBuildInputs = [ autoreconfHook util-linux ];

  doCheck = true;

  nativeCheckInputs = [ openssl cacert ];
  ...
  meta = with lib; {
    ...
    changelog =
      "https://github.com/wolfSSL/wolfssl/releases/tag/v${finalAttrs.version}-stable";
