...
let
  ...
  unaffectedByFodSandboxEscape = [ "2.3" "2.16" "2.18" "2.19" "2.20" ];
in { stdenv, autoconf-archive, autoreconfHook, bash, bison, boehmgc, boost
, brotli, busybox-sandbox-shell, bzip2, callPackage, coreutils, curl
, docbook_xsl_ns, docbook5, editline, flex, git, gnutar, gtest, gzip, jq, lib
, libarchive, libcpuid, libgit2, libsodium, libxml2, libxslt, lowdown, man
...
let
  self = stdenv.mkDerivation {
    ...
    nativeBuildInputs =
      [ pkg-config autoconf-archive autoreconfHook bison flex jq ]
    ...
    installCheckInputs = lib.optionals atLeast221 [ git ]
      ++ lib.optionals atLeast219 [ man ];
    ...
    meta = with lib; {
      ...
      knownVulnerabilities = lib.optional
        (!builtins.elem (lib.versions.majorMinor version)
          unaffectedByFodSandboxEscape && !atLeast221) "CVE-2024-27297";
    };
  };
in self
