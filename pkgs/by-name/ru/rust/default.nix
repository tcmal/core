...
, bootstrapHashes, selectRustPackage, rustcPatches ? [ ], llvmShared
, llvmSharedForBuild, llvmSharedForHost, llvmSharedForTarget
, llvmPackages # Exposed through rustc for LTO in Firefox
}:
{ stdenv, lib, newScope, callPackage, CoreFoundation, Security
, SystemConfiguration, pkgsBuildBuild, pkgsBuildHost, pkgsBuildTarget
, pkgsTargetTarget, makeRustPlatform, wrapRustcWith, path }:
...
let
  ...
  fastCross = (stdenv.buildPlatform == stdenv.hostPlatform)
...
in {
  ...
  inherit (lib')
    ...
    toRustTargetForUseInEnvVars envVars;
  ...
  packages = {
    ...
    stable = lib.makeScope newScope (self:
      let
        ...
        bootstrapRustPackages = if fastCross then
      ...
      in {
        ...
        rustc-unwrapped = self.callPackage ./rustc.nix ({
          ...
          inherit llvmShared llvmSharedForBuild llvmSharedForHost
            llvmSharedForTarget llvmPackages fastCross;
