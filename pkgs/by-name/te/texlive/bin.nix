...
, libXext, perl, perlPackages, python3Packages, pkg-config, cmake, ninja
, libpaper, graphite2, zziplib, harfbuzz, potrace, gmp, mpfr, mupdf-headless
, brotli, cairo, pixman, xorg, clisp, biber, woff2, xxHash, makeWrapper
, shortenPerlShebang, useFixedHashes, asymptote, biber-ms, tlpdb }@args:
...
let
  ...
  coreBigPackages =
    [ "metafont" "mflua" "metapost" "luatex" "luahbtex" "upmendex" "xetex" ]
  ...
  common = {
    ...
    binToOutput = lib.listToAttrs (lib.concatMap (n:
      map (v: {
        name = v;
        value = builtins.replaceStrings [ "-" ] [ "_" ] n;
      }) binPackages.${n}.binfiles or [ ]) (builtins.attrNames binPackages));
...
in rec { # un-indented
  ...
  core = stdenv.mkDerivation rec {
    ...
    patches = [
      (fetchpatch {
        ...
        url =
          "https://tug.org/svn/texlive/trunk/Build/source/texk/ttfdump/libttf/hdmx.c?r1=57915&r2=69520&view=patch";
        stripLen = 2;
  ...
  core-big = stdenv.mkDerivation {
    ...
    patches = [
      ...
      (fetchpatch {
        ...
        url =
          "https://bugs.debian.org/cgi-bin/bugreport.cgi?att=1;bug=1009196;filename=lua_fixed_hash.patch;msg=45";
      ...
      (fetchpatch {
        ...
        url =
          "https://github.com/TeX-Live/texlive-source/commit/ad8702a45e317fa9d396ef4d50467c37964a9543.patch";
      ...
      (fetchpatch {
        ...
        url =
          "https://github.com/TeX-Live/texlive-source/commit/6ace460233115bd42b36e63c7ddce11cc92a1ebd.patch";
      ...
      (fetchpatch {
        ...
        url =
          "https://raw.githubusercontent.com/void-linux/void-packages/master/srcpkgs/LuaJIT/patches/e9af1abec542e6f9851ff2368e7f196b6382a44c.patch";
    ...
    buildInputs = core.buildInputs
      ++ [ core cairo harfbuzz icu graphite2 libX11 potrace ];
    ...
    */
    ...
    outputs = [ "out" "dev" "man" "info" ]
      ++ (builtins.map (builtins.replaceStrings [ "-" ] [ "_" ])
        coreBigPackages)
  ...
  context = stdenv.mkDerivation rec {
    ...
    src = fetchurl {
      url =
        "https://tug.org/svn/texlive/trunk/Master/source/luametatex-${version}.tar.xz?revision=67034&view=co";
    ...
    meta = with lib; {
      description =
        "The LUAMETATEX engine is a follow up on LUATEX and is again part of CONTEXT development";
  ...
  dvisvgm = stdenv.mkDerivation rec {
    ...
    src = assert lib.assertMsg (version == texlive.pkgs.dvisvgm.version)
      ...
      fetchurl {
        url =
          "https://github.com/mgieseki/dvisvgm/releases/download/${version}/dvisvgm-${version}.tar.gz";
  ...
  asymptote = args.asymptote.overrideAttrs (finalAttrs: prevAttrs: {
    ...
    src = fetchurl {
      url =
        "mirror://sourceforge/asymptote/${finalAttrs.version}/asymptote-${finalAttrs.version}.src.tgz";
    ...
    patches = [
      (fetchpatch {
        ...
        url =
          "https://github.com/vectorgraphics/asymptote/commit/7e17096b22d18d133d1bc5916b6e32c0cb24ad10.patch";
