{ lib, fetchurl, runCommand, writeShellScript

# script interpreters
, bash, jdk, perl, python3, ruby, snobol4, tk
...
{ pname, revision, version ? toString revision, extraRevision ? ""
, extraVersion ? "", sha512, mirrors, fixedHashes ? { }, postUnpack ? ""
, postFixup ? "", stripPrefix ? 1, license ? [ ], hasHyphens ? false
, hasInfo ? false, hasManpages ? false, hasRunfiles ? false, hasTlpkg ? false
, extraNativeBuildInputs ? [ ], ... }@args:
...
let
  ...
  outputs = lib.optional hasBinfiles "out" ++ lib.optional hasRunfiles "tex"
    ...
    lib.optional hasTlpkg "tlpkg" ++ lib.optional hasManpages "man"
  ...
  mkContainer = tlType: tlOutputName: sha512:
    let
      fixedHash =
        fixedHashes.${tlType} or null; # be graceful about missing hashes
      # the basename used by upstream (without ".tar.xz" suffix)
      # tlpkg is not a true container but a subfolder of the run container
      urlName = pname
        + (lib.optionalString (tlType != "run" && tlType != "tlpkg")
          ".${tlType}");
      urls = map (up: "${up}/archive/${urlName}.r${toString revision}.tar.xz")
        mirrors;
    ...
    in removeAttrs container [ "out" ] // outputDrvs;
  ...
  containers = rec {
    ...
    out = runCommand "${name}" {
      ...
      buildInputs = let outName = builtins.replaceStrings [ "-" ] [ "_" ] pname;
      in [
      ...
      scriptsFolder = lib.optionals (hasRunfiles && tex ? outPath)
        (map (f: tex.outPath + "/scripts/" + f)
          (lib.toList args.scriptsFolder or pname));
      # binaries info
    ...
    man = removeAttrs (runCommand "${name}-man" {
      inherit meta texdoc;
    ...
    info = removeAttrs (runCommand "${name}-info" {
      inherit meta texdoc;
