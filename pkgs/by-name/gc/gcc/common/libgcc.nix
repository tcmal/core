{ lib, stdenv, version, langC, langCC, langJit, enableShared, targetPlatform
, hostPlatform, withoutTargetLibc, libcCross }:
...
([
  ...
  (pkg:
    pkg.overrideAttrs (previousAttrs:
      lib.optionalAttrs (targetPlatform != hostPlatform
        && (enableShared || targetPlatform.isMinGW) && withoutTargetLibc) {
  ...
  (let
    targetPlatformSlash = if hostPlatform == targetPlatform then
      ""
    else
    ...
    enableLibGccOutput = (!stdenv.targetPlatform.isWindows
