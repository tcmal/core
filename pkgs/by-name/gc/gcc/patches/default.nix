{ lib, stdenv, langC, langAda, langObjC, langObjCpp, langD, langFortran, langGo
, reproducibleBuild, profiledCompiler, langJit, staticCompiler, enableShared
, enableLTO, version, fetchpatch, majorVersion, targetPlatform, hostPlatform
, noSysDirs, buildPlatform, fetchurl, withoutTargetLibc, threadsCross }:
...
in [ ] ++ optional (atLeast6 && !atLeast12) ./fix-bug-80431.patch
++ optional (targetPlatform != hostPlatform) ./libstdc++-target.patch
++ optionals (noSysDirs)
([ (if atLeast12 then ./gcc-12-no-sys-dirs.patch else ./no-sys-dirs.patch) ]
  ++ ({
    "13" =
      [ ./13/no-sys-dirs-riscv.patch ./13/mangle-NIX_STORE-in-__FILE__.patch ];
    "12" =
      [ ./no-sys-dirs-riscv.patch ./12/mangle-NIX_STORE-in-__FILE__.patch ];
...
++ optional langFortran (if atLeast12 then
  ./gcc-12-gfortran-driving.patch
else
  ./gfortran-driving.patch) ++ optional atLeast7 ./ppc-musl.patch
++ optional is12
./12/lambda-ICE-PR109241.patch # backport ICE fix on ccache code
  ...
  == hostPlatform && hostPlatform == targetPlatform) ({
    ...
    "11" = [
      (fetchpatch {
        ...
        url =
          "https://github.com/iains/gcc-11-branch/compare/ff4bf326d03e750a8d4905ea49425fe7d15a04b8..gcc-11.4-darwin-r0.diff";
...
++ optional (atLeast6 && !atLeast13 && !withoutTargetLibc
  && targetPlatform.isMinGW && threadsCross.model == "mcf")
...
++ optionals (is11) [
  ./11/gcc-issue-103910.patch
]

## gcc 10.0 and older ##############################################################################

++ optional (langAda && (is9 || is10)) ./gnat-cflags.patch ++ optional (is10
  && buildPlatform.system == "aarch64-darwin" && targetPlatform
...
./6/0001-Fix-build-for-glibc-2.31.patch ++ optionals (is6 && langAda) [
  ./gnat-cflags.patch
...
++ optional (!atLeast7 && hostPlatform.isDarwin
...
++ optional (is6 && hostPlatform.isDarwin)
./6/libstdc++-disable-flat_namespace.patch

## gcc 4.9 and older ##############################################################################

++ optional (!atLeast6) ./parallel-bconfig.patch ++ optionals (is49) [
...
./gfortran-darwin-NXConstStr.patch ++ optionals is48 [
  (fetchpatch {
