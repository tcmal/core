...
, enableLTO ? stdenv.hostPlatform.hasSharedLibraries, texinfo ? null
...
, withoutTargetLibc ? false, gnused ? null
, cloog # unused; just for compat with gcc4, as we override the parameter on some places
, buildPackages, pkgsBuildTarget, libxcrypt, disableGdbPlugin ? !enablePlugin
  || (stdenv.targetPlatform.isAvr && stdenv.hostPlatform.isDarwin
    && stdenv.hostPlatform.isAarch64), nukeReferences, callPackage
, majorMinorVersion, darwin

# only for gcc<=6.x
, langJava ? false, flex, boehmgc ? null, zip ? null, unzip ? null
...
let
  inherit (lib)
    callPackageWith filter getBin
    # maintainers
    makeLibraryPath makeSearchPathOutput mapAttrs optional optionalAttrs
    optionals optionalString pipe platforms versionAtLeast versions;
  ...
  disableBootstrap = atLeast11 && !stdenv.hostPlatform.isDarwin
    && (atLeast12 -> !profiledCompiler);
  ...
  callFile = callPackageWith ({
    ...
    inherit majorVersion version buildPlatform hostPlatform targetPlatform
      targetConfig patches crossMingw stageNameAddon crossNameAddon;
    ...
    inherit binutils buildPackages cloog withoutTargetLibc disableBootstrap
      ...
      perl pkgsBuildTarget profiledCompiler reproducibleBuild staticCompiler
      stdenv targetPackages texinfo threadsCross which zip zlib;
  } // optionalAttrs (!atLeast7) {
    inherit boehmgc flex gnat-bootstrap gtk2 langAda langJava libICE libSM
      libX11 libXi libXrandr libXrender libXt libXtst libart_lgpl pkg-config
      unzip x11Support xorgproto javaAwtGtk xlibs;
    javaEcj = fetchurl {
...
in assert !atLeast6 -> (cloog != null -> isl != null);

assert langJava -> !atLeast7 && zip != null && unzip != null && zlib != null
  && boehmgc != null && perl != null; # for `--enable-java-home'
...
assert !atLeast7 -> (x11Support
  -> (filter (x: x == null) ([ gtk2 libart_lgpl ] ++ xlibs)) == [ ]);

pipe ((callFile ./common/builder.nix { }) ({
  pname = "${crossNameAddon}${name}";
  inherit version;

  src = if is6 && stdenv.targetPlatform.isVc4 then
    fetchFromGitHub {
      owner = "itszor";
      repo = "gcc-vc4";
  ...
  outputs = if atLeast7 then
    [ "out" "man" "info" ] ++ optional (!langJit) "lib"
  else if atLeast49 && (langJava || langGo
    || (if atLeast6 then langJit else targetPlatform.isDarwin)) then [
      "out"
      "man"
      "info"
    ] else [
  ...
  configureFlags = (callFile ./common/configure-flags.nix { })
    ++ optional (is7 && targetPlatform.isAarch64)
  ...
  buildFlags =
    ...
    if atLeast11 then
      let
        target = optionalString (profiledCompiler) "profiled" + optionalString
  ...
  env = mapAttrs (_: v: toString v) ({
    ...
    NIX_CFLAGS_COMPILE = optionalString (stdenv.cc.isClang && langFortran)
      "-Wno-unused-command-line-argument"
      # Downgrade register storage class specifier errors to warnings when building a cross compiler from a clang stdenv.
      + optionalString (stdenv.cc.isClang && targetPlatform != hostPlatform)
      " -Wno-register";
  } // optionalAttrs (!is7 && !atLeast12 && stdenv.cc.isClang && targetPlatform
    != hostPlatform) { NIX_CFLAGS_COMPILE = "-Wno-register"; }
  ...
  meta = {
    inherit (callFile ./common/meta.nix { })
      homepage license description longDescription platforms
      # maintainers
    ;
    ...
    badPlatforms =
      if targetPlatform != hostPlatform then [ "aarch64-darwin" ] else [ ];
  };
  ...
  preBuild = ''
    makeFlagsArray+=('STRIP=${
      getBin darwin.cctools-port
    }/bin/${stdenv.cc.targetPrefix}strip')
  '';
} // optionalAttrs (!atLeast8) {
  doCheck =
    false; # requires a lot of tools, causes a dependency cycle for stdenv
} // optionalAttrs enableMultilib { dontMoveLib64 = true; } // optionalAttrs
  (((is49 && !stdenv.hostPlatform.isDarwin) || is6) && langJava) {
    postFixup = ''
    ...
    (callPackage ./common/libgcc.nix {
      inherit version langC langCC langJit targetPlatform hostPlatform
        withoutTargetLibc enableShared libcCross;
    })
  ] ++ optionals atLeast11
