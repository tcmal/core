...
stdenv.mkDerivation rec {
  ...
  patches = [ ./enable-static-shared.patch ] ++ lib.optionals stdenv.isAarch32 [
  ...
  makeFlags = [
    ...
    "AEXT=${
      lib.strings.removePrefix "." stdenv.hostPlatform.extensions.staticLibrary
    }"
    "ENABLE_SHARED=${if enableShared then "1" else "0"}"
    "ENABLE_STATIC=${if enableStatic then "1" else "0"}"
  ] ++ lib.optionals enableShared [
    "SOEXT=${
      lib.strings.removePrefix "." stdenv.hostPlatform.extensions.sharedLibrary
    }"
