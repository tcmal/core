{ lib, stdenv, llvm_meta, release_version, cmake, zlib, ncurses, swig3, swig4
, which, libedit, libxml2, libllvm, libclang, python3, version, darwin, lit
, makeWrapper, lua5_3, ninja, runCommand, src ? null, monorepoSrc ? null
, patches ? [ ], enableManpages ? false, ... }:
...
let
  src' = if monorepoSrc != null then
    ...
    src;
  vscodeExt = {
    name = if lib.versionAtLeast release_version "18" then
      "lldb-dap"
    else
      "lldb-vscode";
    version =
      if lib.versionAtLeast release_version "18" then "0.2.0" else "0.1.0";
  };
...
in stdenv.mkDerivation (rec {
  ...
  sourceRoot = lib.optional (lib.versionAtLeast release_version "13")
    "${src.name}/${pname}";
  ...
  passthru.vscodeExtUniqueId =
    "llvm-org.${vscodeExt.name}-${vscodeExt.version}";
  ...
  meta = llvm_meta // {
    ...
    broken = (lib.versionOlder release_version "11" && stdenv.isDarwin
      && stdenv.isAarch64)
  ...
  buildPhase = lib.optionalString (lib.versionOlder release_version "15") ''
    make ${
      if (lib.versionOlder release_version "12") then
        "docs-man"
      else
        "docs-lldb-man"
    }
  ...
  ninjaFlags =
    lib.optionals (lib.versionAtLeast release_version "15") [ "docs-lldb-man" ];
