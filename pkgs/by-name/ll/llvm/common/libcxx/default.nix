{ lib, stdenv, llvm_meta, release_version, monorepoSrc ? null, src ? null
, patches ? [ ], runCommand, substitute, cmake, lndir, ninja, python3
, fixDarwinDylibNames, version
, cxxabi ? if stdenv.hostPlatform.isFreeBSD then libcxxrt else null, libcxxrt
, libunwind, enableShared ? !stdenv.hostPlatform.isStatic }:
...
let
  ...
  cxxCMakeFlags = [ "-DLIBCXX_CXX_ABI=${cxxabiName}" ]
    ...
    [ "-DLIBCXX_HAS_MUSL_LIBC=1" ] ++ lib.optionals
    (lib.versionAtLeast release_version "18" && !useLLVM
      && stdenv.hostPlatform.libc == "glibc" && !stdenv.hostPlatform.isStatic)
  ...
  cmakeFlags = [ "-DLLVM_ENABLE_RUNTIMES=${lib.concatStringsSep ";" runtimes}" ]
    ++ lib.optionals (useLLVM && !stdenv.hostPlatform.isWasm) [
    ...
    ] ++ cxxCMakeFlags ++ lib.optionals (cxxabi == null) cxxabiCMakeFlags;
...
in stdenv.mkDerivation (rec {
  ...
  || lib.versionAtLeast release_version "17") then {
    postPatch = (lib.optionalString (lib.versionAtLeast release_version "14"
