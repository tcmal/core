{ lowPrio, newScope, pkgs, lib, stdenv, cmake, preLibcCrossHeaders, libxml2
, python3, fetchFromGitHub, substitute, substituteAll, overrideCC, wrapCCWith
, wrapBintoolsWith
  ...
  tools = lib.makeExtensible (tools:
    ...
    in {
      ...
      lld = callPackage ../common/lld {
        patches = [ ./lld/gnu-install-dirs.patch ./lld/fix-root-src-dir.patch ];
        inherit llvm_meta;
      };
  ...
  libraries = lib.makeExtensible (libraries:
    ...
    in {
      ...
      openmp = callPackage ../common/openmp {
        patches =
          [ ./openmp/gnu-install-dirs.patch ./openmp/run-lit-directly.patch ];
