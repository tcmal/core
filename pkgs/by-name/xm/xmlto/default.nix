{ fetchurl, fetchpatch, lib, stdenv, libxml2, libxslt, docbook_xml_dtd_45
, docbook_xsl, flex, w3m, bash, getopt, makeWrapper }:
...
stdenv.mkDerivation rec {
  ...
  patches = [
    ...
    (fetchpatch {
      url =
        "https://pagure.io/xmlto/c/8e34f087bf410bcc5fe445933d6ad9bae54f24b5.patch";
      hash = "sha256-z5riDBZBVuFeBcjI++dAl3nTIgOPau4Gag0MJbYt+cc=";
    })
    # Fixes implicit `int` on `ifsense`, which is also an error with clang 16.
    (fetchpatch {
      url =
        "https://pagure.io/xmlto/c/1375e2df75530cd198bd16ac3de38e2b0d126276.patch";
