...
in {
  ...
  compiler = let
    ...
    mkOpenjdkLinuxOnly = path-linux: args:
      ...
      in assert stdenv.isLinux;
      openjdk // {
        headless = openjdk.override { headless = true; };
      };

  in rec {
    adoptopenjdk-8 = mkAdoptopenjdk ./adoptopenjdk-bin/jdk8-linux.nix
      ./adoptopenjdk-bin/jdk8-darwin.nix;

    adoptopenjdk-11 = mkAdoptopenjdk ./adoptopenjdk-bin/jdk11-linux.nix
      ./adoptopenjdk-bin/jdk11-darwin.nix;

    adoptopenjdk-13 = mkAdoptopenjdk ./adoptopenjdk-bin/jdk13-linux.nix
      ./adoptopenjdk-bin/jdk13-darwin.nix;

    adoptopenjdk-14 = mkAdoptopenjdk ./adoptopenjdk-bin/jdk14-linux.nix
      ./adoptopenjdk-bin/jdk14-darwin.nix;

    adoptopenjdk-15 = mkAdoptopenjdk ./adoptopenjdk-bin/jdk15-linux.nix
      ./adoptopenjdk-bin/jdk15-darwin.nix;

    adoptopenjdk-16 = mkAdoptopenjdk ./adoptopenjdk-bin/jdk16-linux.nix
      ./adoptopenjdk-bin/jdk16-darwin.nix;

    adoptopenjdk-17 = mkAdoptopenjdk ./adoptopenjdk-bin/jdk17-linux.nix
    ...
    openjdk8-bootstrap =
      mkBootstrap adoptopenjdk-8 ./openjdk/bootstrap.nix { version = "8"; };

    openjdk11-bootstrap =
      mkBootstrap adoptopenjdk-11 ./openjdk/bootstrap.nix { version = "10"; };

    openjdk13-bootstrap = mkBootstrap adoptopenjdk-13 ./openjdk/12.nix
      (bootstrapArgs // {
    ...
    openjdk14-bootstrap = mkBootstrap adoptopenjdk-14 ./openjdk/13.nix
      (bootstrapArgs // { inherit openjdk13-bootstrap; });

    openjdk15-bootstrap = mkBootstrap adoptopenjdk-15 ./openjdk/14.nix
      (bootstrapArgs // { inherit openjdk14-bootstrap; });

    openjdk16-bootstrap = mkBootstrap adoptopenjdk-16 ./openjdk/15.nix
      (bootstrapArgs // { inherit openjdk15-bootstrap; });

    openjdk17-bootstrap = mkBootstrap adoptopenjdk-17 ./openjdk/16.nix
      (bootstrapArgs // { inherit openjdk16-bootstrap; });

    openjdk18-bootstrap = mkBootstrap adoptopenjdk-17 ./openjdk/17.nix
      (bootstrapArgs // { inherit openjdk17-bootstrap; });

    openjdk8 = mkOpenjdk ./openjdk/8.nix ./zulu/8.nix { };

    openjdk11 =
      mkOpenjdk ./openjdk/11.nix ./zulu/11.nix { openjfx = openjfx11; };
    ...
    openjdk18 = mkOpenjdk ./openjdk/18.nix ./zulu/18.nix {
      inherit openjdk18-bootstrap;
      openjfx = openjfx17;
    };

    openjdk19 = mkOpenjdk ./openjdk/19.nix ./zulu/19.nix {
      openjdk19-bootstrap = temurin-bin.jdk-19;
      openjfx = openjfx19;
    };

    openjdk20 = mkOpenjdk ./openjdk/20.nix ./zulu/20.nix {
    ...
    temurin-bin = recurseIntoAttrs (callPackage (if stdenv.isLinux then
      ./temurin-bin/jdk-linux.nix
    else
      ./temurin-bin/jdk-darwin.nix) { });

    semeru-bin = recurseIntoAttrs (callPackage (if stdenv.isLinux then
      ./semeru-bin/jdk-linux.nix
    else
      ./semeru-bin/jdk-darwin.nix) { });
  };
} // lib.optionalAttrs config.allowAliases {
  jogl_2_4_0 = throw "'jogl_2_4_0' is renamed to/replaced by 'jogl'";
  mavenfod =
    throw "'mavenfod' is renamed to/replaced by 'maven.buildMavenPackage'";
}
