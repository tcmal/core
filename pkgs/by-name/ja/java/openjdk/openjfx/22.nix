{ stdenv, lib, fetchFromGitHub, fetchpatch, fetchurl, runCommand, writeText
, openjdk21_headless, gradle, pkg-config, perl, cmake, gperf, gtk2, gtk3
, libXtst, libXxf86vm, glib, alsa-lib, ffmpeg_4, python3, ruby, withMedia ? true
, withWebKit ? false }:
...
let
  ...
  icuPath = "download/release-${icuVersionWithSep "-"}/icu4c-${
      icuVersionWithSep "_"
    }-data-bin-l.zip";
  icuData = fetchurl {
...
in makePackage {
  ...
  gradleProperties = ''
    ...
    "icuRepositoryURL = file://${icuFakeRepository}"}
  '';
